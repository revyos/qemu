...
void VDSP_HELPER(vmtvru8)(CPUCSKYState *env, uint32_t insn)
{
    ...
    immd = (insn >> CSKY_VDSP_REG_SHI_VRY) & CSKY_VDSP_REG_MASK;
    rx = (insn >> CSKY_VDSP_REG_SHI_VRX) & CSKY_VDSP_GREG_MASK;
    rz = insn & CSKY_VDSP_REG_MASK;
...
void VDSP_HELPER(vstd)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, vz, addr, offset;

    rx = (insn >> CSKY_VDSP_REG_SHI_VRX) & CSKY_VDSP_GREG_MASK;
    vz = insn & CSKY_VDSP_REG_MASK;
    offset = (((insn >> CSKY_VDSP_REG_SHI_VRY) &
              CSKY_VDSP_REG_MASK) << 4) |
             ((insn >> CSKY_VDSP_SIGN_SHI) &
              CSKY_VDSP_REG_MASK);

    size = (insn >> 8) & 0x3;
    width = 8 << size;

    /* TODO: check offset if need shift. */
    addr = env->regs[rx] + (offset << 3);
    /* check align */
    if (addr & ((1 << size) - 1)) {
    ...
    case 64:
        for (i = 0; i < n; i++) {
            csky_cpu_stq_data_ra(env, addr, env->vfp.reg[vz].udspl[i], GETPC());
            addr += 8;
        }
        break;
    default:
...
void VDSP_HELPER(vstq)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, vz, addr, offset;
...
void VDSP_HELPER(vstrd)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, ry, vz, addr, shift;

    rx = (insn >> CSKY_VDSP_REG_SHI_VRX) & CSKY_VDSP_GREG_MASK;
    ry = (insn >> CSKY_VDSP_REG_SHI_VRY) & CSKY_VDSP_GREG_MASK;
    vz = insn & CSKY_VDSP_REG_MASK;
    shift = (insn >> CSKY_VDSP_SOP_SHI_S) & 0x3;

    size = (insn >> 8) & 0x3;
    width = 8 << size;

    addr = env->regs[rx] + (env->regs[ry] << shift);
    /* check align */
    if (addr & ((1 << size) - 1)) {
        helper_exception(env, EXCP_CSKY_ALIGN);
        return;
...
void VDSP_HELPER(vstrq)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, ry, vz, addr, shift;
...
void VDSP_HELPER(vldd)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, vz, addr, offset;
    ...
    case 64:
        for (i = 0; i < n; i++) {
            env->vfp.reg[vz].udspl[i] = csky_cpu_ldq_data_ra(env, addr, GETPC());
            addr += 8;
        }
        break;
    default:
...
void VDSP_HELPER(vldq)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, vz, addr, offset;
...
void VDSP_HELPER(vldrd)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, ry, vz, addr, shift;
...
void VDSP_HELPER(vldrq)(CPUCSKYState *env, uint32_t insn)
{
    int32_t i, n;
    uint32_t size, width, rx, ry, vz, addr, shift;
