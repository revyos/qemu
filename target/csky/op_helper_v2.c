...
void helper_bkpt_exit(CPUCSKYState *env, uint32_t pc)
{
    if (env->exit_bkpt) {
        exit(0);
    } else {
        env->pc = pc;
        helper_exception(env, EXCP_CSKY_BKPT);
    }
}
...
void helper_exception(CPUCSKYState *env, uint32_t excp)
{
    ...
    if (excp == EXCP_CSKY_BKPT || excp == EXCP_DEBUG) {
        if (semihosting_enabled(!cpu_mmu_index(env, false))) {
...
void helper_tag_pctrace(CPUCSKYState *env, uint32_t tb_pc)
{
...
void HELPER(vfp_update_fcr)(CPUCSKYState *env)
{
    ...
    if (!csky_has_feature(env, ABIV2_FPU3)) {
        if ((env->vfp.fcr >> 23) & 0x1) {
            env->vfp.fp_status.default_nan_mode = 0;
        } else {
    ...
    if ((env->cp0.psr & 0x2) != (rx & 0x2)) {
        if (env->features & (CPU_C807 | CPU_C810)) {
            ...
            qemu_log_mask(LOG_GUEST_ERROR,
                          "Only C807 C810 have alternative registers");
        }
    }
...
void helper_rte(CPUCSKYState *env)
{
    ...
    if (env->nvic) {
        csky_nvic_complete_irq(env->nvic);
    }

    helper_exception(env, EXCP_CSKY_INT_EXIT);
}
