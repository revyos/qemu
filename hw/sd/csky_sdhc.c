/*
 * T-HEAD SD Host Controller emulation for CSKY V2.
 *
...
#define TYPE_THEAD_SDHC "csky_sdhc"
#define THEAD_SDHC(obj) OBJECT_CHECK(csky_sdhc_state, (obj), TYPE_THEAD_SDHC)
...
{
    csky_sdhc_state *host = THEAD_SDHC(d);
...
static void csky_sdhc_transfer(csky_sdhc_state *host)
{
    ...
    if (!write) {         /* read a date from sd card */
        while ((fifocnt < 0x80) && (host->Byte_Cnt > 0)) {
            value[0] = sd_read_byte(host->card);
            value[1] = sd_read_byte(host->card);
            value[2] = sd_read_byte(host->card);
            value[3] = sd_read_byte(host->card);
        ...
        while ((fifocnt > 0) && (host->Byte_Cnt > 0)) {
            ...
            sd_write_byte(host->card, value1);
            sd_write_byte(host->card, value1 >> 8);
            sd_write_byte(host->card, value1 >> 16);
            sd_write_byte(host->card, value1 >> 24);
            fifocnt--;
            host->Byte_Cnt -= 4;
        }
    }
...
{
    csky_sdhc_state *s = THEAD_SDHC(dev);
    DriveInfo *dinfo;
