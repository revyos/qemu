...
static void csky_timer_init(Object *obj)
{
    ...
    s->timer[0] = ptimer_init(csky_timer_tick0, s, PTIMER_POLICY_DEFAULT);
    sysbus_init_irq(sbd, &s->irq[0]);

    s->freq[1] = csky_timer_freq;
    s->timer[1] = ptimer_init(csky_timer_tick1, s, PTIMER_POLICY_DEFAULT);
    sysbus_init_irq(sbd, &s->irq[1]);

    s->freq[2] = csky_timer_freq;
    s->timer[2] = ptimer_init(csky_timer_tick2, s, PTIMER_POLICY_DEFAULT);
    sysbus_init_irq(sbd, &s->irq[2]);

    s->freq[3] = csky_timer_freq;
    s->timer[3] = ptimer_init(csky_timer_tick3, s, PTIMER_POLICY_DEFAULT);
