...
static void csky_timer_dummy_init(Object *obj)
{
    ...
    s->timer[0] = ptimer_init(csky_timer_dummy_tick0, s,  PTIMER_POLICY_DEFAULT);
    sysbus_init_irq(sbd, &s->irq[0]);

    s->freq[1] = 1000000000ll;
    s->timer[1] = ptimer_init(csky_timer_dummy_tick1, s, PTIMER_POLICY_DEFAULT);
    sysbus_init_irq(sbd, &s->irq[1]);

    s->freq[2] = 1000000000ll;
    s->timer[2] = ptimer_init(csky_timer_dummy_tick2, s, PTIMER_POLICY_DEFAULT);
    sysbus_init_irq(sbd, &s->irq[2]);

    s->freq[3] = 1000000000ll;
    s->timer[3] = ptimer_init(csky_timer_dummy_tick3, s, PTIMER_POLICY_DEFAULT);
